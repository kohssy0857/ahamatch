...
class loginFollow {
  ...
  String T02_UnitName = "";
  late DocumentReference<Map<String, dynamic>> T06_GeininIdOkuru ;
  ...
  loginFollow(DocumentSnapshot doc) {
    ...
    // ドキュメントのid取得
    ID = doc.id;
    T06_GeininIdOkuru = doc['T06_GeininIdOkuru'];
  }
}
...
class MainModelGeinin extends ChangeNotifier {
  ...
  Future<void> fetchConvention() async {
    ...
    final docs2 = await FirebaseFirestore.instance.collection("T06_Chat").get();
    ...
    final T06Chat  = docs2.docs
        .map((doc) => loginFollow(doc)) 
        .toList();
    this.T06_Chat = T06Chat; 
    notifyListeners(); 
  }
...
class NotificationMane extends StatelessWidget {
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      title: Text("通知画面だよ"),
                      subtitle: Text(T06Chat[index].T06_GeininIdOkuru.id), // 商品名
                      // onTap: () async {
                      //             Navigator.push(
